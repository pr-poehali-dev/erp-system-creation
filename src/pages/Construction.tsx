  ...
  return (
    <div className="space-y-6 max-w-[1400px]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: "Активных домов",
            value: loading ? "—" : String(activeProjects.length),
            icon: "Home",
          },
          {
            label: "Завершено",
            value: loading ? "—" : String(doneProjects.length),
            icon: "CheckCircle",
          },
          {
            label: "В срок",
            value: loading ? "—" : String(onTimeProjects.length),
            icon: "Clock",
          },
          {
            label: "С отставанием",
            value: loading ? "—" : String(lateProjects.length),
            icon: "AlertTriangle",
          },
        ].map((c) => (
          <div
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-dashed border-border p-12 flex flex-col items-center gap-3 text-muted-foreground">
            ...
            <span className="text-[14px] font-medium">Проектов пока нет</span>
                ...
                <div className="px-5 py-4 border-b border-border flex items-center gap-4 flex-wrap">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <span
                        className={`text-[11px] px-2 py-0.5 rounded-full font-medium flex items-center gap-1 ${status.cls}`}
                      >
                    ...
                    <div className="text-hint truncate">
                  ...
                  <div className="text-right text-[13px] shrink-0">
                    ...
                    <div
                      className={`font-medium ${
                        p.days_left <= 7 ? "text-red-500" : "text-foreground"
                      }`}
                    >
                    ...
                    <div className="flex items-center gap-1.5">
                      {p.stages.map((stage, i) => (
                        <div key={i} className="flex-1 min-w-0">
                          <div
                            className={`h-7 rounded flex items-center justify-center text-[10px] font-medium truncate px-1 transition-all ${getStageColor(
                              stage
                            )}`}
                            title={stage.name}
                          >
