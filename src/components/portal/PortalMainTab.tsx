import { ClientAct, ClientPortalDeal, ClientPortalStage, ClientPaymentHistoryItem } from "@/lib/api";
import Icon from "@/components/ui/icon";
import { STAGE_CFG, fmtMoney, fmt, fmtDate } from "./portal.shared";

interface Props {
  deal: ClientPortalDeal;
  stages: (ClientPortalStage & { effective_status?: string })[];
  pendingActs: ClientAct[];
  signedActs: ClientAct[];
  mainPayments: ClientPaymentHistoryItem[];
  extraPayments: ClientPaymentHistoryItem[];
  paid_main: number;
  paid_extra: number;
  balance: number;
  paid_pct: number;
  ...
  hasGantt: boolean;
  signing: number | null;
  onSign: (act: ClientAct) => void;
  onGoToPlan: () => void;
}

export default function PortalMainTab({
  deal,
  stages,
  pendingActs,
  signedActs,
