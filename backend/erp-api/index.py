...
# ─── MATERIAL CATEGORIES (иерархический справочник) ──────────────────────────

def get_material_categories(cur):
    """Плоский список всех категорий (фронт строит дерево сам)."""
    cur.execute(f"""
        SELECT mc.id, mc.name, mc.parent_id, mc.sort_order,
               (SELECT COUNT(*) FROM {SCHEMA}.materials m WHERE m.category_id = mc.id) as materials_count
        FROM {SCHEMA}.material_categories mc
        ORDER BY mc.parent_id NULLS FIRST, mc.sort_order, mc.name
    """)
    cols = [d[0] for d in cur.description]
    return [dict(zip(cols, r)) for r in cur.fetchall()]


def get_category_subtree_ids(cur, category_id: int):
    """Возвращает id категории и всех её потомков (рекурсивно)."""
    cur.execute(f"""
        WITH RECURSIVE sub AS (
            SELECT id FROM {SCHEMA}.material_categories WHERE id = %s
            UNION ALL
            SELECT c.id FROM {SCHEMA}.material_categories c
...
def get_or_create_other_category(cur):
    """Возвращает id корневой категории «Прочее», создаёт если нет."""
    cur.execute(f"""
        SELECT id FROM {SCHEMA}.material_categories
        WHERE parent_id IS NULL AND lower(name) = lower('Прочее') LIMIT 1
    """)
    row = cur.fetchone()
    if row:
        return row[0]
    cur.execute(f"""
        INSERT INTO {SCHEMA}.material_categories (name, parent_id, sort_order)
        VALUES ('Прочее', NULL, 9999) RETURNING id
    """)
    return cur.fetchone()[0]
...
def rename_material_category(cur, cat_id: int, name: str):
    name = (name or '').strip()
    if not name:
        raise ValueError("Название обязательно")
    cur.execute(f"""
        UPDATE {SCHEMA}.material_categories SET name=%s, updated_at=now() WHERE id=%s
    """, (name, cat_id))
    return True


def move_material_category(cur, cat_id: int, new_parent_id, sort_order):
    """Сменить родителя и/или порядок. Защита от перемещения в собственное поддерево."""
    new_parent_id = int(new_parent_id) if new_parent_id else None
    if new_parent_id is not None:
        if new_parent_id == cat_id:
            return False, "Категория не может быть родителем самой себя"
        subtree = set(get_category_subtree_ids(cur, cat_id))
        if new_parent_id in subtree:
            return False, "Нельзя переместить категорию в её же подкатегорию"
    sets = ["parent_id=%s"]
    vals = [new_parent_id]
    if sort_order is not None:
        sets.append("sort_order=%s"); vals.append(int(sort_order))
    sets.append("updated_at=now()")
    vals.append(int(cat_id))
    cur.execute(f"UPDATE {SCHEMA}.material_categories SET {', '.join(sets)} WHERE id=%s", vals)
    return True, None


def delete_material_category(cur, cat_id: int):
    """Удалить категорию. Запрещено если есть подкатегории или используется в materials."""
    cur.execute(f"SELECT COUNT(*) FROM {SCHEMA}.material_categories WHERE parent_id=%s", (cat_id,))
    if cur.fetchone()[0] > 0:
        return False, "Нельзя удалить: есть вложенные подкатегории"
    cur.execute(f"SELECT COUNT(*) FROM {SCHEMA}.materials WHERE category_id=%s", (cat_id,))
    if cur.fetchone()[0] > 0:
        return False, "Нельзя удалить: категория используется в материалах"
    cur.execute(f"DELETE FROM {SCHEMA}.material_categories WHERE id=%s", (cat_id,))
    return True, None


DEFAULT_CATEGORY_TREE = """Товары и услуги
ААА
Аренда (Строительного оборудования)
Доставка (логистика)
.Доставка Газель (4 метра 1.5 тонны)
.Доставка манипулятор. 5 тн
Инженерные системы
.Вентиляция
...
...Круглые
...Плоские
..Диффузоры и анемостаты
..Ревизионные люки
...Металличсекие люки
...Пластиковые
..Редукторы и переходники
...100 фитинги
...125 фитинги
...55х110 фитинги
...Клапан-дефлектор
...Площадка торцевая
...Соединитель эксцентрический
...Фланцы
...
..Аксиальные фитинги
..Арматура запорная
...Арматура запорная для радиаторов
...Вентили и задвижки для воды
...Коллекторы для воды и комплектующие
...Обратные клапана
...
...Чугунные элементы
.Газоснабжение
..Краны шаровые газовые
..Шланги газовые
.Герметизация резьбовых соединений
..Лен сантехнический
..Нить герметизирующая
..Сантехпаста
..Фумлента
.Дренажные системы
..Геоматериалы
...
..Отводы и сливы
..Сантехнические прокладки и манжеты
..Трапы сливные
..Трубы канализационные
...Внутренние
...Наружние
..Фитинги канализационные
...Фитинги 110 внутренняя
...Фитинги 110 наружная
...Фитинги 50 внутренняя
.Насосы и аксесуары
..Гидроаккумуляторы и комплектующие
...
.Отопление
..Баки расширительные
..Газовые и электро котлы
..Греющий кабель и комплектующие
..Коллекторы для отопления
...Коллекторные группы
...Коллекторы распределительные
...Комплектующие для коллекторов отопления
...Узлы насосно смесительные
..Комплектующие для отопления
...
..Теплый пол
...Теплый пол водяной и комплектующие
...Теплый пол электрический и комплектующие
.Система обогрева и кондиционирования
..Системы кондиционирования
..Системы обогрева
.Тепло-звуко изоляция
Инструмент
.Абразивные материалы
..Наждачная бумага
..Шлифовальные бруски,блоки
..Шлифовальные сетки,губки
...
..Емкости для краски
..Кисти
...Кисти бюджет (Сибртех)
...Кисти для водных красок (синяя серия)
...Кисти для лаков(желтая серия)
...Кисти для работ по дереву (зеленая серия)
...Кисти для специальных работ
...Кисти для эмали (красная серия)
...Кисти Премиум( Matrix)
...Кисть ракля,макловица
.Монтажный инструмент
..Клеевые пистолеты
..Крюки дял вязки арматуры
..Отвертки
...Matrix
...Sparta
...Сибртех
...
...Коронки алмазные
..Масла и смазки
..Оснастка для перфораторов и дрелей
...
.....Длинная серия
.....Короткая серия
..Расходники для клеевых пистолетов
..Расходники для точильных станков
..Расходники для ушм
...Алмазные диски
...Зачистные диски
...
...Чашки и коронки
...Шлифовальные диски
..Расходники для шлифмашин
...Шлифовальные диски ШМ
...Шлифовальные ленты
..Расходные материалы и оснастка для пил
...Пилки для лобзиков
...Пильные диски
...
...Плиткорезы
...Стеклорезы
...Строительные ножи и лезвия
.Сварочное оборудование
..Аксессуары для сварки
..Газовые баллоны
..Газовые горелки
..Расходные материалы для сварочных работ
...Проволока
...Электроды
..Сварочные аппараты
...
...Комплектующие для плиткореза и инструмент
...Крестики
...
...Скребки
...Шпатели
....Sparta (дерево) стандарт
....Sparta (пластик) стандарт
....Сибртех (премиум) двухкомпонентная
....Шпатели для затирки
....Шпатели для обоев
....Шпатели (ЗУБ)
....Шпателя для разных работ
..Штукатурные кельмы и ковши
.Электро инструмент
Интерьер и отделка
.Двери и окна
..Двери
...
....Двери межкомнатные экошпон
...Двери технические специальные
...Дверные коробки,доборы
..Дверная фурнитура
...Дверные ручки и комплектующие
...Доводчики дверные
...Завертки и накладки
...
...Петли дверные
....Навесы гаражные
....Петля дверная (накладная)
....Петля накладная садовая
...Упоры для дверей
...Шпингалеты,щеколды,задвижки
....Засовы
....Шпингалеты
..Окна и комплектующие
...Жалюзи оконные
...Карнизы для штор и комплектующие
...Окна пластиковые
...Оконные откосы
...Отливы для окон
...Подоконники
...
...Шторы рулонные
.Декоративные панели
..Комплектующие для монтажных панелей
..Панели МДФ
...Панель МДФ Latat Модерн
...Панель МДФ Latat Нью
...Панель МДФ Uberture Бриз
...Панель МДФ Uberture Лорд
...Панель МДФ Союз Elegance
...Панель МДФ Союз Классик
..Панели ПВХ
...Панели ПВХ потолочные
...Панели ПВХ Термоперевод ТТМ
....ТТМ
...Фартук кухонный ПВХ
.Керамогранит, плитка
..Кафельная плитка
..Керамогранит
...Cersanit
...Primavera Формат 60х60 Matt
...Yasmin
...Керамогранит ТШ Киргизия
.Мебельная фурнитура
..Крючки,вешалки
..Ручки мебельные
..Фурнитура для шкафа
.Напольные покрытия
..SPC Каменно-полимерный композит
...BONKEEL
....Just
....Next
....Slab
...SPS-LINE (выводиться удалить)
...SPS-STYLE
....ROYCE
....SPC-ENJOY
....SPC- JERSEY
...
....SPS-SENSE
...TARKEET
....Коллекция Funky House
..Ковровое покрытие
..Ламинат и аксессуары
..Линолеум
..Плинтусы, напольные пороги
..Плитка кварцвиниловая
..Подложки под напольное покрытие
..Придверные коврики
.Обои
...
..Рым-гайки
..Стальные тросы
..Стропы,багажные ремни
..Телрепы
..Цепи оцинкованные
.Саморезы и шурупы
..Саморезы гипсокартон-дерево
...Разновес(черные саморезы)
...Фасованные черные саморезы (100,500 гр)
..Саморезы ЖЦ
...ЖЦ разновес
...ЖЦ фасовка
..Саморезы кровельные
...
..Хомуты червянные
Лкм,пены,герметики
.Антисептики, пропитки по дереву
..Антисептики защитно-декоративные
..Морилки по дереву
...Водная
...Неводная
..Пропитки для бани и сауны
..Пропитки огнебиозащитные
.Герметики
..Герметики акриловые
...
.Поддоны
.Полотенце сушители
.Пьедесталы и раковины
..Комплектующие для раковин
..Пьедесталы под раковину
..Раковины для ванной
..Сифоны и сливы
.Товары для душа,смесители
..Душевые акссесуары
...
..Рулонная гидроизоялция
..Цементная гидроизоялция
...
.Листовые пластики
..Комплектующие (к листовым пластикам)
...Комплектующие (прозрачные)
...Комплектующие (цветные)
...
.Металлопрокат
..Арматура рифленная
..Комплектующие для металлопроката
...Комплектующие для арматуры
...Комплектующие для профильной трубы
..Косынка металлическая
..Круг
..Лист алюминевый
..Лист стальной
...
..Мешки для мусора
..Пленки поэлителеновые
..Скотчи, ленты, стрейч пленки
.Стеклопластик
..Арматура
..Сетка
.Стеновые материалы
...
..Пенополистерол
..Пенополистерол (Экструдированный)
..Поролон ТШИ
...
..500х500х50
..Бордюр, водосток
..Примерно-песчаные изделия
.Фасадные материалы
..Сайдинг металлический
...Сайдинг металлический бревно
...Сайдинг металлический брус
...Сайдинг металлический корабельная доска
..Сайдинг пластиковый
...Сайдинг LUX под дерево
...Сайдинг LUX под камень
...Сайдинг стандарт
..Фасадные панели
..Фасадная плитка
Товары для бани и сауны
.Аксессуары для бани и сауны
.Баки для воды и теплообменники
...
def seed_material_categories(cur, tree_text: str):
    """Идемпотентный импорт дерева категорий из текста с отступами.
    Уровень вложенности определяется числом ведущих точек (.) либо пробелов/табов (2 пробела = уровень).
    Если категория с таким именем уже есть у данного родителя — переиспользуем её.
    Если tree_text пустой — используется встроенное дерево DEFAULT_CATEGORY_TREE.
    """
    if not tree_text or not tree_text.strip():
        tree_text = DEFAULT_CATEGORY_TREE
    lines = [ln for ln in (tree_text or "").split("\n") if ln.strip()]
    # стек пар (indent, category_id)
    stack = []
    created = 0
    order_counter = {}  # parent_id -> next sort_order
    for ln in lines:
        stripped = ln.rstrip()
        # Уровень: число ведущих точек, либо (если точек нет) число ведущих пробелов/табов / 2
        dot_level = len(stripped) - len(stripped.lstrip("."))
        if dot_level > 0:
            indent = dot_level
            ...
            indent = spaces // 2
            name = stripped.strip()
        # поднимаемся по стеку пока текущий indent <= indent верхушки
        ...
        if parent_id is None:
            cur.execute(f"""
                SELECT id FROM {SCHEMA}.material_categories
                WHERE parent_id IS NULL AND lower(name)=lower(%s) LIMIT 1
            """, (name,))
        else:
            cur.execute(f"""
                SELECT id FROM {SCHEMA}.material_categories
                WHERE parent_id=%s AND lower(name)=lower(%s) LIMIT 1
            """, (parent_id, name))
        row = cur.fetchone()
        if row:
            cat_id = row[0]
            ...
            cur.execute(f"""
                INSERT INTO {SCHEMA}.material_categories (name, parent_id, sort_order)
                VALUES (%s,%s,%s) RETURNING id
            """, (name, parent_id, order_counter[key]))
            cat_id = cur.fetchone()[0]
            created += 1
        stack.append((indent, cat_id))
...
def get_materials(cur):
    """Список материалов с категорией."""
    cur.execute(f"""
        SELECT m.id, m.name, m.unit, m.supplier_category, m.is_active, m.created_at,
...
def create_material(cur, body):
    ...
    cur.execute(f"""
        INSERT INTO {SCHEMA}.materials (name, unit, supplier_category, category_id)
        VALUES (%s,%s,%s,%s) RETURNING id, name, unit
    """, (name, unit, cat, category_id))
    row = cur.fetchone()
...
def update_material(cur, mid, body):
    ...
    if 'category_id' in body:
        ...
        sets.append("category_id=%s"); vals.append(int(cid) if cid else None)
    if not sets: return False
...
def get_invoices(cur, category_id=None, category_filter=None):
    """Список счетов с поставщиком, материалом и категорией материала.
    Фильтрация:
      - category_id: показать счета, где материал в этой категории ИЛИ её подкатегориях
      - category_filter='none': материал без категории (новые AI-счета)
      - category_filter='other': материал в корневой категории «Прочее»
    """
    where = ""
    params = []
    if category_filter == 'none':
        where = "WHERE m.category_id IS NULL"
    elif category_filter == 'other':
        other_id = get_or_create_other_category(cur)
        sub_ids = get_category_subtree_ids(cur, other_id)
        where = "WHERE m.category_id = ANY(%s)"
        params.append(sub_ids)
    elif category_id:
        sub_ids = get_category_subtree_ids(cur, int(category_id))
        where = "WHERE m.category_id = ANY(%s)"
        params.append(sub_ids)
...
def handler(event: dict, context) -> dict:
    ...
    ROUTES = {"deals", "projects", "procurement", "payments", "kcompany", "dashboard", "clients", "staff",
              ...
              "material_categories", "extract_pdf_text"}
    resource = qs.get("r", "")
    if not resource:
    ...
    try:
            ...
            if method == "GET":
                ...
                if action == "create":
                    try:
                        ...
                        if "uq_matcat" in str(e).lower() or "unique" in str(e).lower():
                            return err("Категория с таким названием уже есть у этого родителя")
                        raise
                    conn.commit()
                    return ok(result, 201)
                elif action == "rename":
                    rename_material_category(cur, int(body["id"]), body.get("name", ""))
                    conn.commit()
                    return ok({"ok": True})
                elif action == "move":
                    ok_res, error = move_material_category(
                        cur, int(body["id"]), body.get("parent_id"), body.get("sort_order"))
                    if error: return err(error)
                    conn.commit()
                    return ok({"ok": ok_res})
                elif action == "delete":
                    ok_res, error = delete_material_category(cur, int(body["id"]))
                    if error: return err(error)
                    conn.commit()
                    return ok({"ok": ok_res})
                elif action == "seed":
        ...
        # ── INVOICES ───────────────────────────────────────────────────────────
            ...
            if method == "GET":
                cat_id = qs.get("category_id")
                cat_filter = qs.get("category_filter")
                return ok(get_invoices(
                    cur,
